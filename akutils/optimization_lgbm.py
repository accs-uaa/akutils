...
def cross_val_nmse_regressor(estimator, data, all_variables, predictor_all, target_field, stratify_field, group_field):
    ...
    inner_results = pd.DataFrame(columns=all_variables + inner_split + ['y_predicted'])
    ...
    while inner_cv_i <= inner_cv_length:
        ...
        y_regress_inner = inner_train_iteration[target_field[0]].astype('int32').copy()
        ...
        inner_test_iteration = inner_test_iteration.assign(y_predicted=prediction_inner)
    ...
    y_regress_predicted = inner_results['y_predicted'].astype(float).copy()
...
def optimize_lgbmclassifier(init_points, n_iter, data, targets, groups):
    """
...
def optimize_lgbmregressor(init_points, n_iter, data, all_variables, predictor_all, target_field, stratify_field, group_field):
    """
